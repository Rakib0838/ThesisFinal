...
\begin{figure}[H]
\centering
\includegraphics[width=1.0\textwidth]{figure/Corgi3}
\end{figure}
When a computer precess an image, it considers the image an array of pixel's values. Each of this pixel can be considered as point and it's intensity is described by a range of numerical numbers between 0 to 255. The pixel's values in the image are not sufficient enough information to us when we perform image classification. Therefore we compute the features from the image which represents for a pixel or a whole object in the image. A feature of a point or an object is some characteristic that can help us to tell it apart from another different point or object. In other words, features help us to group similar points or objects together. Examples of features are color components, pixel intensities, mean value of intensities, shape, edges, gradient magnitude, gradient direction, length, area. The calculation of feature depends on the type of application, we want to use for. There are a lots of algorithms are available to extract feature from the image in the area of image processing and computer vision. Each of theses algorithms uses different techniques to compute the different features from the images. As already mentioned that we are interested to instant and shape classification of the objects. Therefore we pick two state of the of the art algorithms for this thesis work. For instant object classification, We use CNN ( convolution neural network ) algorithms to calculate the features from RGB image. \\\\ On the other hand for the shape classification, we use VFH ( view point feature histogram ) algorithm to calculate the descriptor from RGB-D image. The other name of RGB-D image is called point cloud or depth image. The point cloud is a set of points in a three-dimensional coordinate system intended to represent the external surface of an object. When We calculate feature from this point cloud, which is called 3D features or descriptor. Descriptors are more complex and precise signatures of a point or object, and encode a lot of information about the surrounding geometry. Descriptors can be two types local descriptors and global descriptors. Local descriptors are computed for individual points and they do not describe the notion of what an object is, instead they just describe how the local geometry around that point. On the other hand, global descriptors encode object geometry. They are not computed for individual points, rather than for a whole cluster that represents an object. Because of this, a extra preprocessing step (segmentation) is required, in order to retrieve possible objects candidates from the point cloud.
Global descriptors are generally used for object recognition, classification, geometric analysis such as shape and pose estimation.
\section{Convolution Neural Network}
\section{Viewpoint Feature Histogram}
The VFH (Viewpoint Feature Histogram) is a global descriptor. It is derived from the FPFH (First Point Feature Histogram). The VFH consists of two parts: a viewpoint direction component and an extended FPFH component. To calculate the first one, the object's centroid is computed, which is the point that results from averaging the X, Y and Z coordinates of all points. After that, the vector between the viewpoint (the position of the sensor) and the centroid is computed, and normalized. Finally, for all points in the cluster, the angle between their normal and this vector is calculated, and the result is binned into an histogram. The vector is translated to each point when calculating the angle cause it makes the descriptor scale invariant. The viewpoint direction component calculation is shown graphically in figure below.
...
Machine learning is a branch of artificial intelligence and deals with the question how to make machines able to learn from data. The main idea is to enable machine to make intelligent decisions and predictions, based on previous experiences. Algorithms of machine learning need interdisciplinary knowledge and often combine with topics of mathematics, statistics, pattern recognition and more. In general, any machine learning algorithm can be divided into one of the two major types called supervised and unsupervised learning. 
...
\chapter{Result Evaluation Metrics}
\section{Classification Evaluation Method}
...
As robosherlock use UIAM analysis engine to process the data every algorithm in such a pipeline needs to implement as abstract annotator. After adding it to the .xml file define the analysis engine, it will be called based on it's  position in the sequence of the annotators. Since all the perception algorithms in \textbf{Robosherlock} are build as annotators, we also build each classifier in the same way in order to work with \textbf{Robosherlock} pipe line. These classifier annotators are placed in the pipeline in a specific order and classify the object hypotheses, which are generated by the other annotators in the pipeline. The classifier annotator takes inputs as specific predictive model and annotate each hypotheses with a class label more precisely the object name or shape of the object in case of shape classification. The usages of the classifier annotator are given in index A.3 and the flow diagram is show below. 
\begin{figure}[H]
\centering
\tikzstyle{background}=[draw,rectangle,rounded corners,fill=red!50,text width=4em,text centered,minimum height=6mm]
\tikzstyle{block}=[draw, fill=blue!20, rectangle, minimum height=3em, minimum width=6em]
...
\subsection{Practical Implementation of the Algorithms}
\subsubsection{Feature extraction Part}
For implementing the feature extraction algorithms we two libraries or frame work.
We use the open sources library called point cloud library (PCL) for implementing the view point feature extraction algorithms. This library is a framework which provides the functionality to work with depth image (point cloud) taken from senor like kinect. It contains algorithms to simple task like filter out unwanted portion of the cloud to extract single objects from the cloud. The second algorithm is CNN from deep learning framework caffe.  

\subsubsection{Classification Part}
For implementing the machine learning part of this thesis we  use open source computer vision (OpenCV). The library has a class module for deals with all th popular machine learning algorithms. All the algorithms are coded as class and the are derived from a based class. The base or parent class contains all the methods and variables which are common in all the derived class (for Ex. load or same methods). Most of the machine learning algorithms are implemented by two methods called \textbf{train} and \textbf{predict} according to specific algorithms. These methods parameters are varies as the requirements of the algorithms. the train method takes the training data and its label as matrix. The predict method takes the test data as a vector and generate the prediction based of the predictive model. The details about the significant parameters are given below. Besides the machine learning stuffs, the library provides all the necessary classes and function to process the 2D image. The library contains the algorithms to simple filter the image to advanced object detection algorithms.
